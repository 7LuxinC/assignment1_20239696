...
public class ConcreteUserREST {
	...
	public void update_ConcreteUserSelective(final de.ls5.dywa.generated.rest.types.ConcreteUser value, final de.ls5.dywa.generated.entity.dime__HYPHEN_MINUS__models.app.ConcreteUser obj) {
		...
		// If values were not specified, ignore them
		if (effectiveValue.islMSSet()) {
			checkVersion(value, obj);
			if (effectiveValue.getlMS() == null) {
				effectiveObj.setlMS(null);
			}
			else {
				final de.ls5.dywa.generated.entity.dime__HYPHEN_MINUS__models.app.LMS
				 entity;
				final de.ls5.dywa.generated.rest.types.LMS
				 item = effectiveValue.getlMS();
			
				// create new
				if (item.getDywaId() == info.scce.dime.util.Constants.DYWA_ID_CREATE_NEW) {
					if (! (item instanceof de.ls5.dywa.generated.rest.types.LMS)) {
						throw new java.lang.IllegalArgumentException("Runtime type does not match model type");
					}
					...
					final long id = LMSREST.create(dywaName);
					item.setDywaId(id);
				
					//update_LMSSelective
					this.LMSREST.update_LMSSelective(item);
					entity = this.LMSController.read(item.getDywaId());
				}
				// create transient
				else if (item.getDywaId() == info.scce.dime.util.Constants.DYWA_ID_TRANSIENT) {
					if (! (item instanceof de.ls5.dywa.generated.rest.types.LMS)) {
					...
					entity = this.LMSREST.copyToTransient(item);
				}
				// lookup regular object
				else {
					//update_LMSSelective
